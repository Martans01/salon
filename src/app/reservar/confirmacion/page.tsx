...
export default function ConfirmacionPage() {
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <Link href="/reservar" className="text-pink-500 hover:underline">
            Hacer una reserva
          </Link>
        </div>
      </div>
  ...
  const whatsappMessage = encodeURIComponent(
    `Hola! Acabo de reservar una cita en ${BUSINESS_INFO.name}.\n\n` +
    `Nombre: ${confirmation.client_name}\n` +
    (confirmation.branch_name ? `Sucursal: ${confirmation.branch_name}\n` : '') +
    (confirmation.barber_name ? `Barbero: ${confirmation.barber_name}\n` : '') +
    ...
    <div className="min-h-screen bg-black">
      <div className="max-w-lg mx-auto px-4 py-12">
        ...
        >
          ...
          <h1 className="text-2xl font-bold text-white mb-2">Cita Reservada</h1>
          <p className="text-zinc-400">Tu cita ha sido registrada exitosamente</p>
        </motion.div>
        ...
        <p className="text-center text-zinc-600 text-xs mt-6">
          El barbero confirmará tu cita por WhatsApp o teléfono
        </p>
      </div>
    </div>
  )
